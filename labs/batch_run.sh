...
do
    ...
    python3 -m flowsim.labs.run_simulator -o ${series} -p "${out_path}"/*datanum_${i}.json -s datarun --noplot --path "${out_path}" ;
    python3 -m flowsim.labs.gaussian_processes.gp_in_practice -i "${measures}" -n $i -p "${out_path}"/datarun_"${series}"_datanum_$i.csv --silentplot --beautify --path "${out_path}" -s prediction_$i ;
